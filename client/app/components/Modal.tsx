...
const Modal = ({ cancel, confirm, title, desc }: { cancel: () => void, confirm: () => void, title: string, desc: string }) => {
  return (
    <div className='fixed inset-0 bg-black/40 flex justify-center items-center transition-all ease-in-out duration-100 z-99'>
      <div className='flex max-w-xl w-full flex-col justify-center items-center rounded-lg py-[50px] bg-white'>
        ...
        <div className='flex gap-5'>
          ...
          <button className="rounded-full bg-primary px-8 py-2 cursor-pointer font-semibold text-white hover:bg-primary/90" onClick={confirm}>Confirm</button>
        </div>
      </div>
    </div>
  )
